...
import {
  HoogleDocView as HoogleDocViewT,
  IProps as DocProps,
} from './hoogle-doc-view'
import {
  HoogleWebView as HoogleWebViewT,
  IProps as WebProps,
} from './hoogle-web-view'
...
export function createDocView(props: DocProps = {}): HoogleDocViewT {
  ...
  const {
    HoogleDocView,
  }: { HoogleDocView: typeof HoogleDocViewT } = require('./hoogle-doc-view')
...
export function createWebView(props: WebProps = {}): HoogleWebViewT {
  ...
  const {
    HoogleWebView,
  }: { HoogleWebView: typeof HoogleWebViewT } = require('./hoogle-web-view')
  return new HoogleWebView(props)
}

async function showDoc(ed: TextEditor, func: (sym: ISymbol) => void) {
  const token = ed.tokenForBufferPosition(
    ed.getLastCursor().getBufferPosition(),
  )
...
async function reallyActivate(_state: never) {
  ...
  disposables.add(
    atom.commands.add('webview.ide-haskell-hoogle-web', {
      'ide-haskell-hoogle:web-go-back': (ev) =>
        (ev.currentTarget as WebviewTag).goBack(),
      'ide-haskell-hoogle:web-go-forward': (ev) =>
        (ev.currentTarget as WebviewTag).goForward(),
    }),
    atom.commands.add('atom-text-editor', {
      'ide-haskell-hoogle:show-doc-for-symbol': async ({ currentTarget }) =>
        showDoc((currentTarget as TextEditorElement).getModel(), openDoc),
      'ide-haskell-hoogle:show-web-doc-for-symbol': async ({ currentTarget }) =>
        showDoc((currentTarget as TextEditorElement).getModel(), openWeb),
    }),
  )
}
